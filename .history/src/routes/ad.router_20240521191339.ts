...
adRouter.get("/active/true", async (req: Request, res: Response) => {
    ...
    try {
        ...
        res.status(404).send(`Unable to find matching document with is: ${query.isActive}`);
    }
});

{/* Get ad by new status */}

adRouter.get("/new/true", async (req: Request, res: Response) => {
    ...
    try {
        ...
        res.status(404).send(`Unable to find matching document with isNew: ${query.isNew}`);
    }
});
